...
job_title_style = S(
    "jobtitle", fontName="Helvetica-Bold", fontSize=10, textColor=BLACK, leading=13
)
...
def rule():
    return HRFlowable(
        width="100%", thickness=0.5, color=RULE, spaceAfter=4, spaceBefore=0
    )
...
for label, val in skills_data:
    row = Paragraph(f"<b>{label}</b>&nbsp;&nbsp;&nbsp;{val}", skills_style)
    story.append(row)
    story.append(sp(1.5))
...
# Navodita
story += exp_header(
    "Backend Developer Intern", "Navodita Infotech", "Feb 2024 – Mar 2024"
)
story.append(
    bullet(
...
# InternIQ
story += exp_header("Web Developer Intern", "InternIQ", "Mar 2024 – Apr 2024")
story.append(
    bullet(
...
def proj_header(title, link_or_badge, stack):
    ...
    if stack:
        rows.append(
            Paragraph(stack, S("stack", fontSize=8.5, textColor=MUTED, leading=12))
        )
    rows.append(sp(4))
    return rows
...
travelid_block += [
    ...
    Paragraph(
        "Rust, Axum, Subxt, React, TypeScript, PostgreSQL, Polkadot",
        S("stack", fontSize=8.5, textColor=MUTED, leading=12),
    ),
    sp(3),
]
travelid_block.append(
    bullet(
...
story.append(
    bullet("Member of Google Developers Club; attended hackathons and workshops.")
)
story.append(sp(2))
story.append(bullet("1st place in college-wide SQL mobile app competition."))
story.append(sp(2))
story.append(
    bullet("Published two Android apps on the Play Store independently, end-to-end.")
)
story.append(sp(5))
...
story.append(sp(2))
story += exp_header("SSC", "Ravindra Bharathi Public School", "2019 – 2020")
story.append(sp(6))
